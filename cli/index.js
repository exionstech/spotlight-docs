...
program
  .version("1.0.0")
  .description("CLI to create a new Spotlight project")
  .argument(
    "<project-directory>",
    "Directory to create the new Spotlight project"
  )
  .action(async (projectDirectory) => {
    ...
    const { template } = await inquirer.prompt([
      {
        ...
        name: "template",
        message:
          "Which version of the Spotlight template would you like to use?",
    ...
    const repo = `github.com/exionstech/spotlight-docs/${folder}`;
    const emitter = degit(repo);
    const projectPath = path.resolve(process.cwd(), projectDirectory);

    console.log(
      `Creating a new Spotlight project in ${projectPath} from the ${folder} folder...`
    );
    ...
    try {
      ...
      spinner.succeed(
        `Spotlight project successfully created in ${projectPath}!`
      );
